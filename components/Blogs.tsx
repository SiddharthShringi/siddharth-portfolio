...
export default function Blogs({ posts }: { posts: PostMetadata[] }) {
  return (
    <div className="flex flex-col gap-4 ">
      {posts.map((post) => (
        <div
          className="border-b mx-5 hover:border-muted-foreground transition-transform duration-300 ease-out hover:scale-99"
          key={post.slug}
        >
          <Link href={`/blog/${post.slug}`} className="flex flex-col gap-2 py-2 px-2 transition">
            ...
            <div className="hidden md:flex items-center justify-between gap-6 p-2">
              <div className="flex flex-1 flex-col space-y-2 max-w-lg lg:max-w-xl">
                <h2 className="text-xl font-semibold font-sans text-foreground transition-colors cursor-pointer!">
                  {post.title}
                </h2>
                {post.description && (
                  <p className="line-clamp-2 font-normal text-sm font-sans text-muted-foreground tracking-wide">
                    {post.description}
                  </p>
                )}
                {post.publishedAt && (
